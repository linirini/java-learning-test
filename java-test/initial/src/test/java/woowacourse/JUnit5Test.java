...
public class JUnit5Test {
    ...
    class TestAnnotationTest {
         ...
         * 따라서 해당 메서드는 테스트 메서드가 아니기 때문에, 테스트 메서드로서의 역할을 수행하지 않습니다.
         * 모든 테스트 결과는 void or exception으로 반환되며 일관성 있고 신뢰성 있는 테스트 환경을 유지할 수 있다.
         */
        @Test
    ...
    class DisplayNameAnnotationTest {
         ...
         */
        @Test
        @DisplayName("애노테이션을 붙여서 경고를 제거한다.")
    ...
    class AssertEqualsMethodTest {
         ...
         * assertEquals보다는 AssertJ를 사용하는 것이 가독성 있는 코드가 되는 것일까?
         */
        @Test
        ...
        void assertEquals_메서드로_두_값이_같은지_비교한다() {
            ...
            if (expected != actual) {
                throw new RuntimeException("expected: <" + 3 + "> but was: <" + a + b + ">");
            }
        ...
        void assertEquals_메서드로_두_값이_같은지_비교하며_테스트가_실패했을_때_출력되는_메시지를_지정한다() {
            ...
            if (expected != actual) {
                throw new RuntimeException("두 값이 일치하지 않습니다. ==> expected: <" + 3 + "> but was: <" + a + b + ">");
            }
        }
        ...
        void assertNotEquals_메서드로_두_값이_다른지_비교한다() {
            ...
            if (unexpected == actual) {
                throw new RuntimeException("expected: not equal but was: <" + a + b + ">");
            }
        }
    }
    ...
    class AssertSameMethodTest {
        ...
        void assertSame_메서드로_두_객체가_같은지_비교한다() {
            ...
            if (actual == actual) {
                throw new RuntimeException("expected: not equal but was: <" + object + ">");
            }*/
        }
    }
    ...
    class AssertThrowsMethodTest {
        ...
        void assertThrows_메서드로_특정_예외가_발생하는지_비교한다() {
            ...
            assertThrows(RuntimeException.class, () -> causeException());
    ...
    class AssertAllMethodTest {
        ...
        void assertAll_메서드로_여러_검증_코드를_한_번에_실행한다() {
            ...
            //assertAll을 사용하지 않으면 처음 테스트가 실패하는 라인에서 종료된다.
            //assertAll로 한번에 실행함으로써 틀린 케이스를 모두 확인할 수 있다.
            /*assertAll(() -> assertEquals(1 + 2, 3),
                    () -> assertEquals(2 + 3, 5),
                    () -> assertEquals(7 * 3, 21),
                    () -> assertEquals(7 * 3 ^ 5, 32),
                    () -> assertEquals(7 * 3 / 5 + 33 / 21, 4),
